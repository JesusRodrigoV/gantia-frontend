...
export class AcelerometerChart implements OnDestroy {
    ...
    effect(() => {
      ...
      if (telemetry) {
        ...
        if (globalWindow) {
          ...
          this.ingestSocketData(timestamp, telemetry.accel_x, telemetry.accel_y, telemetry.accel_z);
        }
      }
    });
  }
  ...
  private initializeChart(): void {
    ...
    const opts: uPlot.Options = {
      ...
      series: [
        ...
        { label: 'X', stroke: '#dc3545', width: 2 },
        { label: 'Y', stroke: '#28a745', width: 2 },
        { label: 'Z', stroke: '#0f4d92', width: 2 },
      ],
    };
  ...
  private ingestSocketData(timestamp: number, accX: number, accY: number, accZ: number): void {
    this.plotData[0].push(timestamp);
    this.plotData[1].push(accX);
    this.plotData[2].push(accY);
    this.plotData[3].push(accZ);
